...
int StitcherCV2::warp_and_composite(const std::vector<cv::Mat>& frames, const std::vector<cv::Mat>& inmasks)
{
    ...
    for (int i = 0; i < num_images; ++i)
    {
        ...
        corners[i] = warper->warp(image, K, cameras[i].R, INTER_LINEAR, BORDER_REFLECT, images_warped[i]);
    ...
    ***************************/
    ...
    for (int img_idx = 0; img_idx < num_images; ++img_idx)
    {
        ...
        if (abs(compose_scale - 1) > 1e-1)
        {
            ...
            resize(full_mask, mask, Size(), compose_scale, compose_scale, INTER_NEAREST);
        }
        else
        {
            img = full_img;
